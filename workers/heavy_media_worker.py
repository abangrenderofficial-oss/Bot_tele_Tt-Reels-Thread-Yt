...
ACTION = os.environ.get("ACTION", "status_hq").strip() or "status_hq"
PROGRESS_MESSAGE_ID = int(os.environ.get("PROGRESS_MESSAGE_ID", "0") or 0)
SOURCE_MESSAGE_ID = int(os.environ.get("SOURCE_MESSAGE_ID", "0") or 0)
MAX_INPUT_MB = int(os.environ.get("HEAVY_MEDIA_MAX_MB", "250") or 250)
...
def progress_text(percent, action=None):
    value = max(1, min(100, int(round(percent))))
    filled = 10 if value >= 100 else min(9, value // 10)
    bar = "▰" * filled + "▱" * (10 - filled)
    title = "🍎 Live Wallpaper sedang diproses..." if (action or ACTION) == "live_wallpaper" else "🔋 Status HQ sedang diproses..."
    return f"{title}\n{bar} {value}%"
...
def encode_status(input_path, output_path, probe):
    ...
    attempts = (1.0, 0.84, 0.70)
    safe_limit = int(47 * MB)
    for index, scale in enumerate(attempts, 1):
        if output_path.exists():
            output_path.unlink()
        video_kbps = max(180, int(plan["video_kbps"] * scale))
...
def encode_live_wallpaper(input_path, video_path, photo_path, probe):
    ...
    run(
        [
            ...
            "-b:v", f"{video_kbps}k", "-maxrate", f"{int(video_kbps * 1.1)}k", "-bufsize", f"{max(1000, int(video_kbps * 2.2))}k",
